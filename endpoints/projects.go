...
func RegisterProjectHandlers(r *mux.Router, db *sql.DB, cfg *config.Config, sp storage_provider.Binary) {
  root := "/projects"
	...
	sr.HandleFunc("/{projectId}", deleteProject(db, cfg, sp)).Methods("DELETE")
}
...
func deleteProject(db *sql.DB, cfg *config.Config, sp storage_provider.Binary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		project.Id = request.Id
		deleteErr := project.Delete(db, sp)
		if deleteErr != nil {
