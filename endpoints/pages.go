...
func randomFilename(cfg *config.Config, ext string, sp storage_provider.Binary) string {
	fname := ""
	...
	for !generatedFName {
		...
		fname = hex.EncodeToString(randBytes)+"."+ext
		fmt.Println("[+++] Generated file name", fname)
		generatedFName = !sp.Exists(fname)
	}
	return fname
}
...
func RegisterPageHandlers(r *mux.Router, db *sql.DB, cfg *config.Config, sp storage_provider.Binary) {
  root := "/projects/{projectId}/releases/{releaseId}/pages"
	...
	sr.HandleFunc("/{pageId}", deletePage(db, cfg, sp)).Methods("DELETE")
}
...
func createPage(db *sql.DB, cfg *config.Config, sp storage_provider.Binary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		if jpgParseErr != nil {
			...
			if pngParseErr != nil {
				...
				filePath = randomFilename(cfg, "png", sp)
			}
		} else {
			// The image is a valid JPG/JPEG image.
			filePath = randomFilename(cfg, "jpg", sp)
		}
...
func deletePage(db *sql.DB, cfg *config.Config, sp storage_provider.Binary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
